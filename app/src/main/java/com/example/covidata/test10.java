    ...
    public void loadTest(View view) {
        switch (view.getId()) {
            case R.id.button:
                ...
                cnt = cnt + 10;
                Intent intent = new Intent(getApplicationContext(), assesmentResult.class);
                intent.putExtra("count", cnt);
                startActivity(intent);
                break;
            case R.id.button2: databaseReference.child("no").setValue(String.valueOf(b+1));
                cnt = cnt + 0;
                Intent intent1 = new Intent(getApplicationContext(), assesmentResult.class);
